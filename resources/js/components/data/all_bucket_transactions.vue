            ...
            <tr v-for="transaction in transactions.data" :key="transaction.id">
              ...
              <td>
                <a
                  class="btn btn-info"
                  href="#"
                  v-if="transaction.category.title === 'Data'"
...
export default {
  ...
  methods: {
    list(page) {
      ...
      this.isloading = true;
      axios
        .get(`/api/bucket-orders?page=${page}&limit=${limited}`, {
          headers: {
    ...
    getResults(page) {
      ...
      axios
        .get(
          `/api/bucket-orders?search=${this.keyword}&field=${this.field}&status=${this.status}&page=${page}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
    ...
    //   console.log(value);
      ...
      if (confirm("Do you really want to " + value + "?")) {
        axios
          .put(`/api/update/bucket-order/confirmed/${id}`, this.form, {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
            },
