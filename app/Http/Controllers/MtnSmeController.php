...
class MtnSmeController extends Controller
{
    ...
    {
        return view("mtn_sme_website.mtn_login");
    }
    ...
    public function validateMtnProfile(Request $request)
    {
        ...
        return view("mtn_sme_website.otp",['first_name' => $firstName,'phone_number' => $phone_number]);
    }
    public function generateMtnOtp(Request $request)
    {
        ...
        return view("mtn_sme_website.verify_otp",['otp_result' => $otp_result,'phone_number' => $phone_number]);
    }
    ...
    public function validateMtnOtp(Request $request)
    {
        ...
        if($response){
            ...
            // fwrite($fp, $response);
            // fclose($fp);
            Setting::where('key','MTN_TOKEN')->update(['value' => $response]);
            ///saving the phone number
            // $fp = fopen(storage_path('app/mtnnum.txt'), 'w');
            // fwrite($fp, $phone_number);
            // fclose($fp);

            Setting::where('key','MTN_SPONSOR_NUMBER')->update(['value' => $phone_number]);


            return view("mtn_sme_website.mtn_success_login",['response' => $response]);

        }else{
            return view("mtn_sme_website.mtn_failed_login",['response' => "An error occurred"]);
    ...
    {
        $phone_number = Setting::where('key','MTN_SPONSOR_NUMBER')->first()->value;
        \Log::info($phone_number);
        ...
        return view("mtn_sme_website.account_active",['balance' => $result, 'smebundles' => $result2]);
    }
    ...
    public function divideAndRoundUp($numerator, $denominator)
    {
        $quotient = $numerator / $denominator;
        return number_format($quotient, 2, '.', '');
           // return $quotient;
    }
