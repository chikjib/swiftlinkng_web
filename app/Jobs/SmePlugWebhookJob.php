...
class SmePlugWebhookJob extends SpatieProcessWebhookJob
{
    ...
    public function handle()
    {
        ...
        $response = $payload;
        \Log::info("SMEPLUG WEBHOOK RESPONSE");
        \Log::info($response);
        ...
        if (!is_null($oldOrder)) {
                ...
                if ($response['transaction']['status'] == "success") {
                    ...
                    $oldOrder->status = 1;
                    $resp = $this->normalizeBundleMessage($response['transaction']['response']);
                   ...
                   // $this->sendTelegramMessage($oldOrder->amount, $oldOrder->phone, $subcategory->pins, "", $ref, $subcategory->telegram, $msg);
                    ...
                    $response_msg = $subcategory->id === 52 ? "Transaction failed" : ($response['transaction']['response'] ?? null);
